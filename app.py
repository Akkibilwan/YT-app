...
def get_channel_id(channel_name_or_url):
    ...
    for id_type, pattern in patterns.items():
        ...
        # If no URL pattern matched, assume input is a name, handle, or maybe ID
        identifier_to_resolve = identifier
        resolve_type = 'search' # Default to search
        if identifier.startswith('@'):
             resolve_type = 'handle'
             identifier_to_resolve = identifier[1:] # Remove '@' for search query
        logger.debug(f"No URL pattern matched. Treating '{identifier}' as name/handle for '{resolve_type}' resolution.")
    ...
    try:
        ...
        if resolve_type in ['handle', 'custom_url', 'search']:
             ...
             params['type'] = 'channel'
             params['q'] = identifier_to_resolve
             ...
             if data.get("items"):
                 ...
                 target_lower = identifier_to_resolve.lower()
                 best_match_id = None
                 for item in data["items"]:
                     ...
                     # Handle might be available in snippet in future API versions? Assume not for now.

                     # Simple matching logic (can be improved)
                     if channel_id:
                         if target_lower == title or target_lower == custom_url:
                             best_match_id = channel_id
                             logger.info(f"Found likely match for '{identifier_to_resolve}' via search: ID {best_match_id} (Title/URL match)")
                             break # Found a strong match
                         elif not best_match_id: # Keep first result as fallback
                              best_match_id = channel_id

                 if best_match_id:
                      logger.info(f"Resolved '{identifier_to_resolve}' via search to ID: {best_match_id}")
                      return best_match_id
                 else:
                      logger.warning(f"Search for '{identifier_to_resolve}' returned items but no channel ID found.")
                      return None # Should not happen if type=channel
             else:
                  logger.warning(f"API search for '{identifier_to_resolve}' returned no items.")
                  return None
    ...
    logger.warning(f"Could not resolve channel identifier: {channel_name_or_url}")
            ...
            if submitted:
                ...
                with st.spinner("Pre-caching recent data for new channels (this might take a moment)..."):
                     for ch in channel_list:
                          try:
                              search_youtube("", [ch["channel_id"]], "3 months", "Both", ttl=7776000) # Long TTL
                          except Exception as e:
                          ...
                          if added_channels_data:
                              ...
                              with st.spinner("Pre-caching data for newly added channels..."):
                                  for ch_data in added_channels_data:
                                      try: search_youtube("", [ch_data["channel_id"]], "3 months", "Both", ttl=7776000)
...
def generate_transcript_with_openai(audio_file):
    ...
    if actual_size > max_size_bytes:
        ...
        try:
            ...
            logger.warning(f"ffmpeg snippet creation with 'copy' failed: {e.stderr.decode(errors='ignore')}. Retrying with re-encoding.")
            ...
            try:
                 ...
                 logger.error(f"ffmpeg snippet re-encoding also failed: {e_reencode.stderr.decode(errors='ignore')}")
    ...
    try:
        ...
        # Rough estimate based on file size (if original size known) or snippet duration
        duration_estimate = snippet_duration_sec if temp_snippet_file else (actual_size / (16*1024) if actual_size > 0 else 0)
...
def summarize_intro_outro(intro_text, outro_text):
    ...
    except (APIError, openai.APIError) as e: # Catch both new/old errors
             ...
             for i, chunk in enumerate(video_id_chunks):
                 ...
                 try:
                      ...
                      for item in items_in_response:
                          ...
                          is_short_definition = (snippet.get("liveBroadcastContent", "none") == "none" and duration_sec <= 60) # Basic check
                          # A more robust check might involve aspect ratio if thumbnails allow, but API doesn't provide easily
...
# =============================================================================
# 9. Comments & Analysis (Keep as is, already integrated)
# =============================================================================
# Functions analyze_comments and get_video_comments remain as previously defined


# =============================================================================
# 10. Retention Analysis (Keep as is, already integrated)
# =============================================================================
# Functions check_selenium_setup, load_cookies, capture_player_screenshot_with_hover,
# detect_retention_peaks, capture_frame_at_time, plot_brightness_profile,
# filter_transcript, check_ytdlp_installed, download_video_snippet remain as previously defined


# =============================================================================
# 14. UI Pages (Keep structure, use list of dicts from search_youtube)
# =============================================================================
...
def show_details_page():
    ...
    with tab_retention:
        ...
        eligibility_msg = ""
        if is_short:
            eligibility_msg = "ℹ️ Retention analysis is less meaningful for Shorts."
                      ...
                      if peak_details:
                           ...
                           for peak_id, details in peak_details.items():
                                ...
                                with col_info:
                                     ...
                                     if snippet_out_path:
                                          # Check if already downloaded (for this duration)
                                          # Note: simplistic check, doesn't account for duration change well
                                          snippet_key_dl = f"{retention_state_prefix}{peak_id}_snippet_downloaded_{snippet_dur}s"
                                                    ...
                                                    if check_ytdlp_installed():
                                                         with st.spinner("Downloading snippet..."):
                                                             try:
                                                                  ...
                                                                  st.session_state[snippet_key_dl] = True
